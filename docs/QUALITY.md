...
2. **Regression detection**: > 15% slowdown versus the `main` baseline. This one is **not** a CI gate: GitHub-hosted runners are too noisy for a 15% threshold to mean anything, so a hosted comparison would either cry wolf on every PR or be widened until it never fires. It is a reference-machine ritual instead — §5 step 11 carries the recipe. On CI, the absolute ceilings in (1) are the enforcement.
...
CI cannot host a 20 GB fixture (a GitHub runner has ~14 GB of SSD), so the CI run is sized by `GLYDE_BENCH_FIXTURE_GB` in `.github/workflows/ci.yml`. That value must always stay **larger than the RAM cap the gate asserts against** — a fixture that fits inside the budget cannot fail this gate for the reason the gate exists, whatever else it proves. The literal 20 GB run is §5 step 12.
...
1. `cargo fmt --check`
2. `cargo clippy --all-targets -- -D warnings`, run against every crate in the workspace
3. `cargo test --workspace` (unit + golden + corpus + property), run against every crate in the workspace
...
Items 2–3 are split across three `ci.yml` jobs rather than run as a single
`--workspace`/`--all-targets` invocation: `lint` and `test` cover `glyde-core`
and `glyde-devtools` on all three OSes, while `gui` separately lints, tests,
and builds `glyde-app`. The split exists because `glyde-app` pulls in `winit`,
which does not compile on a headless Linux runner without the X11/Wayland/GTK
system libraries the `gui` job installs — so `glyde-app` cannot share the
`lint`/`test` jobs' plain runner setup. Every crate is still linted and tested
on every shipped OS; the combined `lint` + `test` + `gui` jobs satisfy the
`--workspace`/`--all-targets` intent above, just split across jobs instead of
...
11. **Benchmark regression check** (§3.2 — CI does not run this): on the reference machine, `git checkout main && cargo bench -p glyde-core -- --save-baseline main`, then, on the release candidate, `cargo bench -p glyde-core -- --baseline main`. Any benchmark more than 15% slower than the `main` baseline blocks the release until it is explained or fixed.
12. **Full-size memory gate**: `cargo run --release -p glyde-devtools --bin generate_fixtures -- --size-gb 20`, then the same command with `--bin memory_gate -- --size-gb 20`. Peak RSS must stay under `min(25% RAM, 4 GB)`. CI runs this same gate at a smaller — but still over-cap — size; this is the run that matches §3 literally.
