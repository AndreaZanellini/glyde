...
- [ ] Create Cargo workspace with `glyde-core`, `glyde-app`, `glyde-devtools` (publish=false) + `rust-toolchain.toml` → ARCH §Workspace layout · proven by: `cargo build --workspace` green in CI
...
- [ ] `tracing` + `tracing-subscriber` + `tracing-appender` init in `glyde-app` (rotating file in OS app-data dir + console) → SPEC §6, ARCH §Logging · proven by: log file appears on run
...
- [ ] Corpus loader + `<name>.expected.json` schema + open→compare harness (behind a feature so assertions activate as ingestion lands) → QUALITY §1 · proven by: manifest test confirms 56 files each have a valid `expected.json`
