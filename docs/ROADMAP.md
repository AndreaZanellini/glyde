...
- [x] Background progressive build emitting partial levels → first meaningful plot ≤ 2 s while indexing continues → SPEC §5 (first-plot), ARCH §pipeline · proven by: first-plot bench + manual · Raw `Dataset` memory is now bounded by the SPEC §5.1 affordability check and the `index::spill` streaming write path (issue #75); a spilled open checkpoints from a capped in-memory preview so SPEC §5's "first plot, any file size" still holds; and peak RSS is now **flat**, not a fraction of file size, since SPEC §2.2's Δt statistics read the tick column in bounded chunks and select their order statistics exactly without materializing the Δt distribution (issue #85) — `memory_gate` measures 10.7 MB on a 2 GB fixture and 10.3 MB on an 8 GB one. Each checkpoint's pyramid now reaches `glyde-app` and is rendered from (issue #80, closed), and is extended incrementally from the previous checkpoint rather than rebuilt from scratch, so a full progressive load is no longer `O(n log n)` of repeated aggregation (issue #90, closed)
- [x] Decimation query `viewport(range, pixels) → min/max per column`; raw samples + point markers when samples < pixels; convergence to true samples → SPEC §3.1 · proven by: decimation golden — spike, convergence, no-alias (un-ignore) · `glyde-app`'s time view now calls `decimate_viewport` every frame against the plot's live pan/zoom bounds, and the raw-samples-plus-markers regime is conditional on `decimate_viewport`'s own convergence rather than unconditional (issue #80, closed) · **known gap, deliberate**: a *completed* load whose storage was spilled still does not get a final pyramid built for it — the view falls back to a viewport-bounded raw scan for that case, correct but not pyramid-accelerated. The original reason (building one would walk a memory-mapped column end to end, making every page resident — issue #88) no longer applies: `dsp::decimation::build_pyramid_streaming` reads a spilled column in bounded chunks, and `ingest::pyramids_for_dataset` routes any spilled dataset through it. What holds the app back now is the pyramid's *own* size — ~9 bytes per sample per column held owned in RAM, proportional to file size — which is issue #102
...
> #85, #88), and the memory gate runs against a 4 GB fixture — larger than the
...
>
> Full history — what was missing, what closed it, and the reasoning behind the
> one judgement call: **`docs/M3-CLOSEOUT.md`**.
