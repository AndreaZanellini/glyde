...
- [x] Level-0 typed spill cache to OS cache dir keyed by path + size + mtime; reopen is instant → ARCH §Index (issue #59), SPEC §5.1 · proven by: `index::level0` round-trip/reopen unit tests + level0/decimation integration test · **core only**: the open path still never reads or writes this cache — only `index::pyramid` is wired in (below) — so a reopen's raw-sample/deep-zoom path and its CSV text parse are not yet instant (issue #92, split from #81)
- [x] Pyramid *level* spill (only Level 0 is spilled so far; reopening rebuilds the pyramid from cached Level 0 rather than loading it too) → ARCH §Index · proven by: spill round-trip unit test + manual reopen + `pyramid_reopen_integration.rs` · **wired into the open path**: `ingest::pyramids_for_dataset_cached` calls `index::pyramid::build_or_open` per numeric column (keyed by `CacheKey::with_column`, new) for a completed, non-spilled load, so a second open of an unchanged file skips rebuilding every column's pyramid aggregation (issue #81, partially closed). Level 0 wiring, and skipping the CSV parse itself, remain open — issue #92
